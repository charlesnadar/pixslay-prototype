[DESIGN.md](https://github.com/user-attachments/files/27471346/DESIGN.md)
...
### OTP Input
```
4 boxes in a row, gap 8px between  ← confirmed 4 digits, not 6
...
Purpose: Make the user feel in control. They understand what will happen before it does.
Tone: Warm, specific, trip-focused. Never "scan", "access", or "everything".

Case 1 — trip timing exists:
  Headline: "Ready to find your trip photos?" — 20px 800 #1A1917
  Copy: "We'll use this trip window to find photos related to [Group name]." — 13px 400 #6B6864
  Primary CTA: "Find my photos" — blue filled 50px
  Ghost CTA: "Adjust timing" — transparent 44px, #6B6864 — same width as primary

  Fields always visible (no expand mechanic):
    2×2 grid, 8px gap:
      Row 1: "From date" | "From time"  (pre-filled from group creation data)
      Row 2: "To date"   | "To time"    (pre-filled from group creation data)
    "From time" auto-focuses on sheet open — users mainly adjust time, not date
    Helper text below fields: "We'll only use this to find photos from the trip." — 11px 400 #A8A49F centered
    Fields feel like lightweight confirmation, not a form — floating labels, no borders until focus
    Primary: "Start syncing" / Ghost: "Cancel" — both same width
    "Start syncing": saves edited values to state.syncTiming, closes sheet, starts sync
    "Cancel" or overlay tap: closes sheet, no save, no sync

Case 2 — trip timing missing:
  Headline: "When did the trip begin?"
  Date + Time fields immediately visible (no expand needed)
  Primary CTA: "Find my photos"
  Ghost CTA: "Skip for now"
...
Rules:
  ...
  "Find my photos" starts sync. Sheet slides down before progress bar begins.
```
...
**10. Only one primary action should visually dominate a screen at a time:**
Never show two blue primary buttons on the same screen simultaneously.
When a bottom sheet is open, the sheet's primary CTA is the only dominant action — the screen behind is dimmed and its CTAs are not reachable.
In multi-state flows (sync, delivery, upload), the sticky bottom area swaps its primary CTA label and state in-place without re-rendering the screen. This keeps the user oriented — same position, updated meaning.
Ghost CTAs (transparent, #6B6864) are always present alongside the primary and are never hidden or greyed — the app never guilt-trips the user.

**9. No scrollbars on horizontal scroll rows:**
Never show a scrollbar on horizontally scrollable containers (contact chips, face chips, filter bars, photo rows).
Mobile users recognise a partially clipped item as a scroll cue — a scrollbar is redundant and looks like a desktop UI pattern.
Always apply `class="hscroll"` to any overflow-x:auto container. The CSS class suppresses the bar across all browsers:
```css
.hscroll { -ms-overflow-style:none; scrollbar-width:none; }
.hscroll::-webkit-scrollbar { display:none; }
```
The last item in any horizontal row should be partially visible (clipped by screen edge) to signal there is more to scroll to.

**10. No scrollbars on primary photo browsing experiences:**
Primary photo browsing experiences should use visually hidden scrollbars for a cleaner and more immersive experience.
Apply `class="pscroll"` to any vertical photo grid container (trip photos, Group pool, Saved, Downloaded, My Photos, gallery screens).
Momentum scrolling is preserved; scrollability is communicated through partially-visible grid rows and continuous content flow — not a scrollbar rail.
```css
.pscroll { -ms-overflow-style:none; scrollbar-width:none;
           -webkit-overflow-scrolling:touch; }
.pscroll::-webkit-scrollbar { display:none; }
```
Do not apply `.pscroll` to form or content screens (registration, profile, OTP, sync, home). Those are functional flows, not immersive browsing.
